...
then
  ...
  apt-get update
  apt-get install -y mongodb-org=6.0.16 mongodb-org-database=6.0.16 mongodb-org-server=6.0.16 mongodb-org-mongos=6.0.16 mongodb-org-tools=6.0.16
...
then
  ...
  # Nightscout needs version 6 of npm.  So, we install that version now effectivwely downgrading it.  
