...
percentile.html = function html(client) {
  ...
  var ret =
      ' <select id="vaxis_max">' +
      '  <option ymx="22">22 400</option>' +
      '  <option ymx="16" selected>16 300</option>' +
      '  <option ymx="14">14 250</option>' +
      '  <option ymx="12">12 200</option>' +
      '  <option ymx="11">11 200</option>' +
      '</select>' +
      '<br>' +
  '<h2>'
...
percentile.report = function report_percentile(datastorage, sorteddaystoshow, options) {
  ...
  $.plot(
    '#percentile-chart', [{
      ...
      yaxis: {
        min: 0,
        max: options.units === 'mmol' ? options.ymx: 400,
        tickColor: '#555',
      },
      grid: {
        markings: [{
