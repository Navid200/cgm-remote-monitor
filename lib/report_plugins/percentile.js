...
percentile.report = function report_percentile(datastorage, sorteddaystoshow, options) {
  // Read Y-Min and Y-Max from user input and save to localStorage
  const yMinInput = document.getElementById('agpYMin');
  const yMaxInput = document.getElementById('agpYMax');

  if (yMinInput && !isNaN(parseFloat(yMinInput.value))) {
    window.localStorage.setItem('agpYMin', parseFloat(yMinInput.value));
  } else {
    window.localStorage.removeItem('agpYMin');
  }

  if (yMaxInput && !isNaN(parseFloat(yMaxInput.value))) {
    window.localStorage.setItem('agpYMax', parseFloat(yMaxInput.value));
  } else {
    window.localStorage.removeItem('agpYMax');
  }
