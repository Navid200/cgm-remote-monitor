...
'<h2>'
  ...
  + '  <label for="agpYMin">Y-Min:</label>'
  + '  <input type="number" step="any" id="agpYMin" style="width: 60px;" value="' + (window.localStorage.getItem('agpYMin') || '') + '">'
  + '  <label for="agpYMax">Y-Max:</label>'
  + '  <input type="number" step="any" id="agpYMax" style="width: 60px;" value="' + (window.localStorage.getItem('agpYMax') || '') + '">'
  + '  <button id="saveAgpYBounds">Apply</button>'
  + '</div>'
  + '<script>'
  + '  document.getElementById("saveAgpYBounds").addEventListener("click", function () {'
  + '    const yMin = parseFloat(document.getElementById("agpYMin").value);'
  + '    const yMax = parseFloat(document.getElementById("agpYMax").value);'
  + '    if (!isNaN(yMin)) { window.localStorage.setItem("agpYMin", yMin); } else { window.localStorage.removeItem("agpYMin"); }'
  + '    if (!isNaN(yMax)) { window.localStorage.setItem("agpYMax", yMax); } else { window.localStorage.removeItem("agpYMax"); }'
  + '    window.location.href = window.location.origin + window.location.pathname + "#percentile";'
  + '  });'
  + '</script>'
  ;
...
percentile.report = function report_percentile(datastorage, sorteddaystoshow, options) {
  ...
  const yMinUser = parseFloat(window.localStorage.getItem('agpYMin'));
  const yMaxUser = parseFloat(window.localStorage.getItem('agpYMax'));
  const defaultYMin = 0;
  const defaultYMax = options.units === 'mmol' ? 22 : 400;
  $.plot(
    '#percentile-chart', [{
      ...
      yaxis: {
        min: !isNaN(yMinUser) ? yMinUser : defaultYMin,
        max: !isNaN(yMaxUser) ? yMaxUser : defaultYMax,
        tickColor: '#555',
      },
      grid: {
        markings: [{
