...
percentile.html = function html(client) {
  ...
  var ret =
      ' <select id="vaxis_max">' +
      '  <option ymx="22" ymx2="400">22x400</option>' +
      '  <option ymx="16" ymx2="300"selected>16x300</option>' +
      '  <option ymx="14" ymx2="250">14x250</option>' +
      '  <option ymx="13" ymx2="200">12x200</option>' +
      '</select>' +
      '<br>' +
  '<h2>'
...
percentile.report = function report_percentile(datastorage, sorteddaystoshow, options) {
  ...
  $.plot(
    '#percentile-chart', [{
      ...
      yaxis: {
        min: 0,
        max: options.units === 'mmol' ? options.width : options.height,
        tickColor: '#555',
      },
      grid: {
        markings: [{
