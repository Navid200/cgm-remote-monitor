...
percentile.html = function html(client) {
  ...
  var agpYAxisDefault = {
  'mg/dl': {
    agp_ymin: client.settings.thresholds.agpYAxisBottom
    , agp_ymax: client.settings.thresholds.agpYAxisTop
    }
  , 'mmol': {
  agp_ymin: client.utils.scaleMgdl(client.settings.thresholds.agpYAxisBottom)
  , agp_ymax: client.utils.scaleMgdl(client.settings.thresholds.agpYAxisTop)
    }
  };
  var defaults = agpYAxisDefault[client.settings.units.toLowerCase()];
  ...
  + '</div>'
  + '<br>'
  + translate('AGP Y axis limits') + ': '
  + translate('Low') + ' '
  + '<input id="rp_agpyaxisbottom" type="number" size="3" value="' + defaults.agp_ymin + '"> - '
  + translate('High') + ' '
  + '<input id="rp_agpyaxistop" type="number" size="3" value="' + defaults.agp_ymax + '">'
  ;
  $('#rp_agpyaxisbottom').val(agpYAxisDefault[client.settings.units.toLowerCase()].agp_ymin);
  $('#rp_agpyaxistop').val(agpYAxisDefault[client.settings.units.toLowerCase()].agp_ymax);
...
percentile.report = function report_percentile(datastorage, sorteddaystoshow, options) {
  ...
  var minutewindow = 10; //minute-window should be a divisor of 60

  var data = datastorage.allstatsrecords;
  options.agpYAxisBottom = parseFloat($('#rp_agpyaxisbottom').val().replace(',', '.'));
  options.agpYAxisTop = parseFloat($('#rp_agpyaxistop').val().replace(',', '.'));
  var ymin = options.agpYAxisBottom;
  var ymax = options.agpYAxisTop;
