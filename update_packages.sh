...
if [ "$whichpack" = "" ]
then
  sudo apt-get -y install screen nano qrencode file inetutils-ping nginx python3-certbot-nginx inetutils-ping ca-certificates apt-transport-https lsb-release gnupg build-essential
fi
